...
const uint8_t maxPayloadSizeDwellLimited[8] = {0, 0, 11, 53, 125, 242, 242, 242};

// Channels by ism band
ChannelParams_t Channels[MAX_AS_SINGLE_BAND_CHANNELS];

static const int8_t rxWindowOffset[] = {-33, -50, -58, -62, -66, -68, -15, -2};

// Tx power possibilities by ism band
static int8_t txPower923[] = {16, 14, 12, 10, 8, 6, 4, 2};
...
uint8_t maxEirpValueEncoded = 0;
uint8_t maxEirpValue = 0;
static const uint8_t maxEirpDecodeArray[16] = {8, 10, 12, 13, 14, 16, 18, 20, 21, 24, 26, 27, 29, 30, 33, 36}; 
...
static inline void UpdateReceiveWindow1DataRate (bool dlDwellTimeFlag);

static inline uint8_t MaxValue (uint8_t value1, uint8_t value2);

static inline uint8_t MinValue (uint8_t value1, uint8_t value2);

static inline void UpdateMaxTxPower (uint8_t maxEirpVal);

static inline uint8_t decodeMaxEirp (uint8_t maxEirpValueEnc);
...
void LORAWAN_Init(RxAppDataCb_t RxPayload, RxJoinResponseCb_t RxJoinResponse) // this function resets everything to the default values
{
    ...
    LORAWAN_Reset (ISM_BAND_ASIA_DEFAULT);
...
void LORAWAN_Reset (IsmBand_t ismBandNew)
{
    ...
    loRa.maxChannels = MAX_AS_SINGLE_BAND_CHANNELS; 
    ...
    //Enable Dwell time by default
    loRa.macStatus.uplinkDwellTimeFlag = ENABLED;
    loRa.macStatus.downlinkDwellTimeFlag = ENABLED;
}
...
void LORAWAN_TxDone(uint16_t timeOnAir)
{
    if (loRa.macStatus.macPause == DISABLED)
    {
        ...
        {           
            ...
            if(Channels[0].dutyCycle != 0)
            {
                Channels[0].channelTimer = ((uint32_t)timeOnAir) * (((uint32_t)DUTY_CYCLE_JOIN_REQUEST + 1) * ((uint32_t)loRa.prescaler) - 1); 
            }
            else
            {
                Channels[0].channelTimer = ((uint32_t)timeOnAir) * (((uint32_t)Channels[0].dutyCycle + 1) * ((uint32_t)loRa.prescaler) - 1);
            }
       ...
       if(SwTimerIsRunning(loRa.dutyCycleTimerId))
        ...
        for (i=0; i < MAX_AS_SINGLE_BAND_CHANNELS; i++)
        {
...
LorawanError_t ValidateDataRate (uint8_t dataRate)
{
    ...
    if ( ((loRa.macStatus.uplinkDwellTimeFlag == ENABLED) && (dataRate < DR2)) || (dataRate > DR7) )
    {
        result = INVALID_PARAMETER;
    }
    ...
    while(ptr < commandsLen)
    {
        ...
        else if (*(ptr) == LINK_ADR_CID)
        {
            ...
            if (ENABLED == loRa.macStatus.adr)
            {
                ...
                if ( (ValidateDataRate (dataRate) == OK) &&  (dataRate >= AS923_TX_MIN_DATARATE) && (dataRate <= AS923_TX_MAX_DATARATE) && (ValidateChMaskDataRate(channelMask, dataRate, redundancy->chMaskCntl) == OK)) 
                {
                    drAck = true;
                }
    ...
    if ( (loRa.macCommands[loRa.crtMacCmdIndex].channelFrequencyAck == 1) && (loRa.macCommands[loRa.crtMacCmdIndex].dataRateRangeAck == 1) )
    {
        if (loRa.lastUsedChannelIndex < 16)
        {
            if (frequency != 0)
            {
                ...
                UpdateDutyCycle (channelIndex, DUTY_CYCLE_DEFAULT_NEW_CHANNEL);
        ...
        else
        {
            if (frequency != 0)
            {
                ...
                UpdateDutyCycle (channelIndex + 16, DUTY_CYCLE_DEFAULT_NEW_CHANNEL);
...
uint8_t* ExecuteTxParamSetupReq (uint8_t *ptr)
{
    EirpDwellTime_t eirpDwellTime;
    
    eirpDwellTime.value = *(ptr++);
    
    loRa.macStatus.downlinkDwellTimeFlag = (bool)(eirpDwellTime.value >> SHIFT5);      //extract Downlink Dwell Time flag (bit 5 from command))
    loRa.macStatus.uplinkDwellTimeFlag = (bool)(eirpDwellTime.value >> SHIFT4);        //extract Uplink Dwell Time flag (bit 4 from command))
    
    //downlinkDwellTimeFlag = eirpDwellTime.downlinkDwellTime;
    //uplinkDwellTimeFlag = eirpDwellTime.uplinkDwellTime;
    
    maxEirpValueEncoded = (eirpDwellTime.value & 0x0F);                   //extract Maximum EIRP (set 0 on the upper nibble)
    
    maxEirpValue = decodeMaxEirp(maxEirpValueEncoded);
    
    UpdateMaxChDataRate(loRa.macStatus.uplinkDwellTimeFlag);
    UpdateMaxTxPower (maxEirpValue);
    
    UpdateReceiveWindow1DataRate (loRa.macStatus.downlinkDwellTimeFlag);
...
static inline void UpdateReceiveWindow1DataRate (bool dlDwellTimeFlag)
{
    uint8_t downlinkMinDataRate;
    uint8_t valueToCompare;
    
    if (dlDwellTimeFlag == true)
    {
        downlinkMinDataRate = DR2;
    }
    else
    {
        downlinkMinDataRate = DR0;
    }
    
    valueToCompare = MaxValue (downlinkMinDataRate, loRa.currentDataRate - rx1DrOffsetArray[loRa.offset]);
    loRa.receiveWindow1Parameters.dataRate = MinValue (DR5, valueToCompare);
...
static inline void UpdateMaxTxPower (uint8_t maxEirpVal)
{
    ...
    for (i=0; i<=7; i++)
    {
        txPower923[i] = maxEirpVal;
        if(maxEirpVal >= 4)
            maxEirpVal = maxEirpVal - 2;
    }
}

static inline uint8_t decodeMaxEirp (uint8_t maxEirpValueEnc)
{
    return maxEirpDecodeArray[maxEirpValueEnc];
}
...
LorawanError_t SearchAvailableChannel (uint8_t maxChannels, bool transmissionType, uint8_t* channelIndex)
{
    ...
    while (randomNumber)
    {
        ...
        {
            if ( ( Channels[i].status == ENABLED ) && ( Channels[0].channelTimer == 0 ) && ( loRa.currentDataRate >= Channels[i].dataRange.min ) && ( loRa.currentDataRate <= Channels[i].dataRange.max ) )
            {
...
void UpdateCfList (uint8_t bufferLength, JoinAccept_t *joinAccept)  
{
    ...
    {
        // 1 is the minimum channel index for single band
        channelIndex = 2;
...
void UpdateDLSettings(uint8_t dlRx2Dr, uint8_t dlRx1DrOffset)
{
    ...
    if (dlRx1DrOffset <= 7) // lhg - please check this value
    {
...
LorawanError_t SelectChannelForTransmission (bool transmissionType)  // transmission type is 0 means join request, transmission type is 1 means data message mode
{
    ...
    {    
        result = SearchAvailableChannel (MAX_AS_SINGLE_BAND_CHANNELS, transmissionType, &channelIndex);
        ...
        if (result == OK)
        {
            ...
            ConfigureRadioTx(loRa.currentDataRate, Channels[channelIndex].frequency);
        }
    }
    else //join message - AS923 Use DR2
    {
        loRa.currentDataRate = DR2; // DR2 is always used for join
        result = SearchAvailableChannel (MAX_AS_SINGLE_BAND_CHANNELS, transmissionType, &channelIndex);
        RADIO_SetSpreadingFactor (SF_10); 
...
static void InitDefault923Channels (void)
{
    ...
    for (i = 2; i < MAX_AS_SINGLE_BAND_CHANNELS; i++)
    {
...
static LorawanError_t ValidateFrequency (uint32_t frequencyNew)
{
    ...
    switch(loRa.ismBand)
    {
        case ISM_AS923_BRUNEI:
        {
            if ( (frequencyNew > FREQ_925000KHZ) || (frequencyNew < FREQ_920000KHZ) )
            {
                result = INVALID_PARAMETER ;
            } 
        } break;
        
        case ISM_AS923_CAMBODIA:
        {
            if ( (frequencyNew > FREQ_925000KHZ) || (frequencyNew < FREQ_923000KHZ) )
            {
                result = INVALID_PARAMETER ;
            } 
        } break;
        
        case ISM_AS923_HONG_KONG:
        {
            if ( (frequencyNew > FREQ_925000KHZ) || (frequencyNew < FREQ_920000KHZ) )
            {
                result = INVALID_PARAMETER ;
        ...
        case ISM_AS923_INDONESIA:
        {
            if ( (frequencyNew > FREQ_923000KHZ) || (frequencyNew < FREQ_920000KHZ) )
            {
                result = INVALID_PARAMETER ;
            } 
        } break;
        
        case ISM_AS923_JAPAN:
        {
            if ( (frequencyNew > FREQ_928000KHZ) || (frequencyNew < FREQ_920000KHZ) )
            {
                result = INVALID_PARAMETER ;
            } 
        } break;
        ...
        case ISM_AS923_NEW_ZEALAND:
        {
            if ( (frequencyNew > FREQ_928000KHZ) || (frequencyNew < FREQ_915000KHZ) )
            {
                result = INVALID_PARAMETER ;
            } 
        } break;
        
        case ISM_AS923_SINGAPORE:
        {
            if ( (frequencyNew > FREQ_925000KHZ) || (frequencyNew < FREQ_920000KHZ) )
            {
                result = INVALID_PARAMETER ;
        ...
        case ISM_AS923_VIETNAM:
        {
            if ( (frequencyNew > FREQ_923000KHZ) || (frequencyNew < FREQ_918000KHZ) )
            {
                result = INVALID_PARAMETER ;
            } 
        } break;
...
static LorawanError_t ValidateChannelId (uint8_t channelId, bool allowedForDefaultChannels)  //if allowedForDefaultChannels is 1, all the channels can be modified, if it is 0 channels 0, 1, 2 and 16, 17, and 18 (dual band) cannot be modified
{
    ...
    if ( (channelId >= MAX_AS_SINGLE_BAND_CHANNELS) ||  ( (allowedForDefaultChannels == WITHOUT_DEFAULT_CHANNELS) && (channelId < 2) ) )
    {
        result = INVALID_PARAMETER ;
    }
...
static LorawanError_t ValidateDlChannelId (uint8_t channelId)
{
    ...
    if ( channelId >= MAX_AS_SINGLE_BAND_CHANNELS )
    {
        result = INVALID_PARAMETER ;
    }
...
static void EnableChannels (uint16_t channelMask, uint8_t channelMaskCntl)
{
    EnableChannels1 (channelMask, channelMaskCntl, 0, MAX_AS_SINGLE_BAND_CHANNELS);
}
...
static void ConfigureRadioTx(uint8_t dataRate, uint32_t freq)
{
    ...
    txPower = txPower923[loRa.txPower];        
